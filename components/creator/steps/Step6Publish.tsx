...
function suggestTime(platforms: Platform[]): string {
  if (platforms.includes('linkedin')) return 'wtorek 12:00'
  return 'środa 19:30'
}
...
export function Step6Publish({ state, onChange, onBack, onClose }: Props) {
  ...
  // ── Brand display ────────────────────────────────────────────────────────────
  const brandName = state.brand.name?.trim() || 'Twoja marka'
  ...
  // ── Save post to DB ───────────────────────────────────────────────────────────
  const saveToDb = async (status: 'PUBLISHED' | 'SCHEDULED', schedAt: string | null) => {
    setSaving(true)
    setSaveError(null)
    try {
      ...
      setSaveError(e instanceof Error ? e.message : 'Nie udało się zapisać posta')
      return false
    } finally {
      setSaving(false)
    }
  ...
  const suggestedTime = suggestTime(platforms)
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[420px] gap-5 text-center">
        <div className="text-[4rem]">{publishNow ? '🚀' : '📅'}</div>
        <h2 className="font-syne text-[2rem] font-[700] tracking-[-0.03em]">
          {publishNow ? 'Post opublikowany!' : 'Post zaplanowany!'}
        </h2>
        <p className="text-muted text-[0.9rem] max-w-[360px]">
          {publishNow
            ? `Twój post jest widoczny na: ${platforms.join(', ')}.`
            : `Post zostanie opublikowany ${schedLabel}.`}
        </p>
      </div>
    )
  }
    ...
    <div>
      ...
      <div className="bg-surface border border-border rounded-[20px] p-8 grid grid-cols-[1fr_300px] gap-10 mb-4 max-[800px]:grid-cols-1">
              ...
              <div className="bg-white rounded-[22px] overflow-hidden text-black">
                ...
                <div className="w-full aspect-square bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460] flex items-center justify-center overflow-hidden">
                  {state.imageUrl
                    ...
                    : <div className="flex flex-col items-center gap-2 opacity-40"><span className="text-[2.5rem]">🖼️</span><span className="text-[0.6rem] text-white">Brak grafiki</span></div>
                  }
                </div>
                <div className="p-3 text-[0.68rem] leading-relaxed text-gray-700">
                  {captionFull
                    ? <><strong className="text-black font-[700]">{brandHandle}</strong>{' '}{captionPreview}{hasMore && <span className="text-[#0095f6]"> więcej</span>}</>
                    : <span className="text-gray-400 italic">Brak treści posta</span>}
                </div>
            ...
            <div className="w-[280px] bg-white rounded-[12px] border border-gray-200 overflow-hidden text-black shadow-lg">
              <div className="flex items-center gap-2.5 p-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[0.6rem] text-gray-400">Teraz · 🌐</div>
                </div>
              </div>
              <div className="px-3 pb-2 text-[0.72rem] leading-relaxed text-gray-800">
                {captionFull
                  ? <>{captionPreview}{hasMore && <span className="text-[#0a66c2]"> ...pokaż więcej</span>}</>
                  : <span className="text-gray-400 italic">Brak treści</span>}
              </div>
              {state.imageUrl && (
          ...
          {previewTab === 'facebook' && (
            <div className="w-[280px] bg-white rounded-[12px] border border-gray-200 overflow-hidden text-black shadow-lg">
              ...
              <div className="px-3 pb-3 text-[0.75rem] leading-relaxed text-gray-800">
                {captionFull
                  ? <>{captionPreview}{hasMore && <span className="text-[#1877f2]"> Zobacz więcej</span>}</>
                  : <span className="text-gray-400 italic">Brak treści</span>}
              </div>
              {state.imageUrl && (
                <div className="w-full aspect-video overflow-hidden">
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* When */}
          <div>
            <FieldLabel>Kiedy opublikować?</FieldLabel>
            <div className="flex gap-2">
              {[
                { label: 'Teraz', val: true },
                { label: 'Zaplanuj', val: false },
              ].map(({ label, val }) => (
                <button
                  key={label}
          ...
          {/* Platforms */}
          <div>
            <FieldLabel>Platforma</FieldLabel>
          ...
          {hashtags.length > 0 && (
            <div>
              <FieldLabel>Hashtagi (kliknij aby usunąć)</FieldLabel>
          ...
          {publishNow ? (
            ...
            >
              {saving ? (
                <><span className="w-4 h-4 rounded-full border-2 border-t-transparent border-bg animate-spin" /> Zapisywanie…</>
              ) : '🚀 Opublikuj teraz'}
            </Button>
          ) : (
            <Button
      ...
      <div className="flex items-center gap-2.5 pt-6 border-t border-border mt-7">
        <Button variant="secondary" onClick={onBack}>← Wróć do grafiki</Button>
        <span className="text-[0.75rem] text-dim ml-auto">
          Post zostanie opublikowany na @{brandHandle}
        </span>
      </div>
    </div>
  )
